...
/**
 * Read endpoint for the Watson "Event Venue-Fee Invoicing" screen.
 * Called cross-project from the Watson browser app (hence cors:true) so it can
 * display Starlabs event data without a client-side secondary Firebase app / auth.
 *
 *  GET  (no params)      -> { events: [{ id, name, start_date, end_date }] }  (end_date desc)
 *  GET  ?eventid=<id>    -> { participants: [{ docid, profileid, productid, product, status }] }
 *                           = the CONFIRMED (approved/attended) event participation requests
 *
 * Name/billing is resolved on the Watson side by matching profileid -> Watson Participants.
 */
...
/**
 * Receives an `e-ticket eligibility` doc from Watson (server-to-server) and
 * mirrors it into Starlabs under the SAME collection + SAME doc id.
 * Called by the Watson `eticketeligibility_sync` trigger.
 */
