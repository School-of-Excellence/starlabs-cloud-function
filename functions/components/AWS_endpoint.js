...
// Allow self-signed TLS on the OpenVidu master (selfsigned CertificateType dev boxes).
...
/**
 * AWS-specific OpenVidu Elastic endpoint.
 *
 * Holds everything tied to the AWS cloud: LiveKit credentials for the AWS master, the ASG/EC2
 * capacity + master-node lifecycle (moved verbatim out of openVidu.js), the AWS state webhook,
 * and S3 signed-URL playback. openVidu.js (the shared LiveKit dispatcher) calls
 * exports.prepareRoom() here when provider === 'aws'.
 */
...
// LiveKit connection credentials for the AWS OpenVidu master.
exports.creds = () => ({
  url: LIVEKIT_URL.value(),
  key: LIVEKIT_API_KEY.value(),
  secret: LIVEKIT_API_SECRET.value(),
});
// Secrets a caller binds to use creds().
exports.SECRETS = [LIVEKIT_URL, LIVEKIT_API_KEY, LIVEKIT_API_SECRET];
// Extra secret a caller must bind to use prepareRoom() (AWS_ACCESS_KEY/AWS_SECRET already listed
// by createOpenViduToken; mediaASGName is the one it doesn't otherwise need).
exports.CAPACITY_SECRETS = [mediaASGName];
...
// Capacity gate + room creation for the AWS ASG-backed cluster. Same behaviour as the original
// createOpenViduToken AWS branch: returns { scaling:true, ... } to signal a 503, or
// { scaling:false, totalInstances } when the room is ready to join.
