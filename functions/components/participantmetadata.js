...
exports.eventparticipationdata_to_pmd = onDocumentWritten("event participation request/{docid}",async (change) => {
  ...
  console.log('request belong to an event : ' , newdoc['eventref']?.parent.id === 'event collection')

  if (change.data.after.exists && newdoc['eventref']?.parent.id === 'event collection') {
    console.log('automatic communication starts');
    try {
      const currentStatus = newdoc["status"]?.toLowerCase() ?? '';
      const allPromises = [];
      let emailTemplateAlias = null;
      let postMarkTemplateId = null;
      let watiTemplate = null;
      let appNotificationTitle = null;
      let appNotificationMessage = null;

      const profileData = (await admin.firestore().doc(`profile_data/${profileid}`).get()).data();
      const event = (await admin.firestore().doc(`arena events/${newdoc['arenaeventid']}`).get()).data() ?? {};

      if ([null, undefined, ''].includes(olddoc) && currentStatus === 'requested') {
        emailTemplateAlias = 'template_for_request_v3';
        postMarkTemplateId = '45296240';
        watiTemplate = 'request_msg';
        appNotificationTitle = `Request Received for ${event['title'] ?? ''}`;
        appNotificationMessage = `We've received your request to participate in ${event['title'] ?? ''}. Our team is reviewing it and will get back to you soon.`;
      
      } else if (([null, undefined, ''].includes(olddoc) && currentStatus === 'approved') || (olddoc['status']?.toLowerCase() === 'requested' && currentStatus === 'approved')) {
        emailTemplateAlias = 'template_for_approve_v1';
        postMarkTemplateId = '45296277';
        watiTemplate = 'approved_msg';
        appNotificationTitle = `You're Approved for ${event['title'] ?? ''}`;
        appNotificationMessage = `Your request to participate in ${event['title'] ?? ''} has been approved. We'll share the next steps and further details with you soon.`;
      }

      if (emailTemplateAlias && postMarkTemplateId) {
        ...
        allPromises.push(
          commonService.createEmailArchiveDocument({
      ...
      if (watiTemplate) {
        ...
        allPromises.push(
          commonService.createWatiArchiveDocument({
      ...
      if (appNotificationMessage && appNotificationTitle) {
      ...
      if (allPromises.length > 0) {
        ...
        console.log('All communication has been sent');
      }
    } catch (error) {
      console.log(error)
    }
